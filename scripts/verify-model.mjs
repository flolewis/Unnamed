...
for(const t of league.teams){
 ...
 for(const s of [baseline,{availability:-15,development:-5,fit:-6},{availability:15,development:5,fit:6}]){
  const o=teamOutlook(t,d,s,45);assert.ok(Math.abs(o.leagueWins.reduce((sum,t)=>sum+t.wins,0)-1230)<1e-8);assert.equal(o.scheduledGames+o.placeholderGames,82);assert.ok(Math.abs(o.dist.reduce((a,b)=>a+b.p,0)-1)<1e-10);
...
const okc=teams['25'],star=okc.players.find(p=>p.history.some(h=>h.year===2026));
const absent=projectPlayers(okc,{...baseline,rotation:{[star.id]:{gp:0,min:48}}}).find(p=>p.id===star.id);assert.equal(absent.projection.total,0);
const overfilled=projectPlayers(okc,{...baseline,rotation:Object.fromEntries(okc.players.map(p=>[p.id,{gp:82,min:48}]))});assert.ok(Math.abs(overfilled.reduce((s,p)=>s+(p.projection?.min??0),0)-240)<1e-8);
const report=JSON.parse(await readFile(new URL('../data/model-v2.json',import.meta.url),'utf8'));assert.equal(report.team.report.n,90);assert.ok(report.minutes.trainingRows>0);assert.ok(report.minutes.validationRows>0);assert.ok(report.fantasyReport.mae>0);
console.log('PASS: joint league has 1,230 wins; zero-game overrides; overfilled rotation normalization; real held-out metrics.');
